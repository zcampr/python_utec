...
data_actual = pd.read_csv("data/data_actual.csv")
# ======================================================
...
data_home_acc = filter_afec
data_home_acc["CANTIDAD_AFECTADOS"] = data_home_acc["CANTIDAD_FALLECIDOS"] + data_home_acc["CANTIDAD_LESIONADOS"]
data_home_acc = data_home_acc.drop(["CANTIDAD_FALLECIDOS", "CANTIDAD_LESIONADOS", "MES", 'PROVINCIA', 'DISTRITO'], axis=1)
data_home_afec = data_home_acc.groupby(["AÑO", 'DEPARTAMENTO'])['CANTIDAD_AFECTADOS'].sum().reset_index()
...
# Cantidad de afectados por causa principal del accidente
filter_principal_afec = registro_siniestros.loc[:, ['AÑO', 'CAUSA_PRINCIPAL', "CANTIDAD_FALLECIDOS", "CANTIDAD_LESIONADOS", "CLASE_ACCIDENTE"]]
filter_principal_afec["CANTIDAD_AFECTADOS"] = filter_principal_afec["CANTIDAD_FALLECIDOS"] + filter_principal_afec["CANTIDAD_LESIONADOS"]
filter_principal_afec = filter_principal_afec.drop(["CANTIDAD_FALLECIDOS", "CANTIDAD_LESIONADOS"], axis=1)
filter_principal_afec = filter_principal_afec.groupby(["AÑO", 'CAUSA_PRINCIPAL'])['CANTIDAD_AFECTADOS'].sum().reset_index()
