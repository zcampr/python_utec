...
# Cabeceras
st.header("ANALISIS ESTADISTICO DE ACCIDENTES DE TRANSITO")

st.divider()

st.subheader("Solo en el 2023 se reportan ya mas de 9000 afectados por accidentes de transito...")
...
with afectados:
    st.tabs(["AFECTADOS EN LOS ULTIMOS 3 AÑOS"])
...
with accidentes:
    st.tabs(["SINIESTROS EN LOS ULTIMOS 3 AÑOS"])
...
with weather_condition:
    ...
    st.tabs(["SINIESTROS POR CONDICION CLIMATICA"])
    if "Todo" in select_dep:
        # Devolver un grafico por todos los departamentos, segun año en FILTROS
...
with time_day:
    ...
    st.tabs(["SINIESTROS POR TIEMPO EN EL DIA"])
    if "Todo" in select_dep:
        # Devolver un grafico por todos los departamentos, segun año en FILTROS
...
with zone:
    ...
    st.tabs(["SINIESTROS POR ZONFICACION"])
    if "Todo" in select_dep:
        # Devolver un grafico por todos los departamentos, segun año en FILTROS
